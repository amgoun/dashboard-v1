  ...
  return (
    <div className="px-4 sm:px-8 pb-8 space-y-5">
      ...
      <div>
        ...
        <p className="text-ink-secondary text-[12.8px] font-inter mt-0.5">Chat with your team</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ height: "calc(100vh - 220px)", minHeight: 480 }}>
        {/* Conversation list */}
        <div className="bg-surface-card rounded-card shadow-card flex flex-col overflow-hidden">
          {/* Search */}
          <div className="p-4 border-b border-white/5">
            <div className="flex items-center gap-2 bg-surface-page rounded-card px-3 py-2">
              ...
              <input
                ...
                className="bg-transparent text-white text-[11px] font-inter outline-none placeholder:text-ink-muted flex-1"
              />
            </div>
          </div>
            ...
            {conversations.map((conv) => (
              ...
              >
                <div className="relative shrink-0">
                  ...
                  {conv.online && (
                    <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-brand rounded-full border-2 border-surface-card" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className={`text-[12.5px] font-inter font-medium ${selected.id === conv.id ? "text-brand" : "text-white"}`}>
                      {conv.name}
                    </span>
                    <span className="text-ink-muted text-[9.5px] font-inter shrink-0">{conv.time}</span>
                  </div>
                  <p className="text-ink-muted text-[11px] font-inter truncate mt-0.5">{conv.preview}</p>
                </div>
                {conv.unread > 0 && (
                  <span className="ml-1 min-w-[18px] h-[18px] bg-danger rounded-full flex items-center justify-center text-white text-[9px] font-inter shrink-0">
                    {conv.unread}
                  </span>
                )}
              </button>
        ...
        {/* Chat window */}
        <div className="lg:col-span-2 bg-surface-alt rounded-card shadow-card flex flex-col overflow-hidden">
          {/* Chat header */}
          <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
            <div className="relative">
              ...
              {selected.online && (
                <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-brand rounded-full border-2 border-surface-alt" />
              )}
            </div>
            <div>
              <p className="text-white text-[13px] font-inter font-medium">{selected.name}</p>
              <p className={`text-[10px] font-inter ${selected.online ? "text-brand" : "text-ink-muted"}`}>
                  ...
                  <div
                    className={`px-4 py-2.5 rounded-2xl text-[12px] font-inter leading-relaxed ${
                      msg.from === "me"
                        ...
                        : "bg-surface-card text-white rounded-bl-sm"
                    }`}
                  >
                    {msg.text}
                  </div>
                  <span className="text-ink-muted text-[9.5px] font-inter px-1">{msg.time}</span>
                </div>
              </div>
            ))}
          </div>
